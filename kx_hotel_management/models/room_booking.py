...
class RoomBooking(models.Model):
    ...
    date_order = fields.Datetime(string="Order Date",
                                 required=True,
                                 copy=False,
                                 help="Creation date of draft/sent orders, Confirmation date of confirmed orders",
                                 default=fields.Datetime.now)
    is_checkin = fields.Boolean(default=False, string="Is Checkin", help="sets to True if the room is occupied")
    maintenance_request_sent = fields.Boolean(default=False, string="Maintenance Request sent or Not", help="sets to True if the maintenance request send once")
    checkin_date = fields.Datetime(string="Check In", help="Date of Checkin", default=fields.Datetime.now())
    checkout_date = fields.Datetime(string="Check Out", help="Date of Checkout", default=fields.Datetime.now() + timedelta(hours=23, minutes=59, seconds=59))
    hotel_policy = fields.Selection([("prepaid", "On Booking"),
                                    ...
                                    default="manual",
                                    string="Hotel Policy",
                                    help="Hotel policy for payment that either the guest has to pay at booking time, check-in or check-out time.",
                                    tracking=True)
    duration = fields.Integer(string="Duration in Days", help="Number of days which will automatically count from the check-in and check-out date.")
    ...
    service_line_ids = fields.One2many("service.booking.line", "booking_id", string="Service", help="Hotel services details provided to Customer and it will included in the main Invoice.")
    ...
    food_order_line_ids = fields.One2many("food.booking.line", "booking_id", string='Food', help="Food details provided to Customer and it will included in the main invoice.")
    ...
    amount_total = fields.Monetary(string="Total Net Amount", store=True, help="The total Amount including Tax", compute='_compute_amount_untaxed', tracking=4)
    amount_untaxed_room = fields.Monetary(string="Gross Amount", help="Untaxed Amount for Room", compute='_compute_amount_untaxed', tracking=5)
    ...
    advanced_payment = fields.Float("Advance Payment")
    currency_id = fields.Many2one('res.currency', 'Currency', required=True,
        default=lambda self: self.env.company.currency_id.id)
    customer_document_ids = fields.One2many('customer.document', 'booking_id', string='Documents')
    ...
    def _compute_amount_untaxed(self, flag=False):
        ...
        room_lines = self.room_line_ids
        booking_list = []
        account_move_line = self.env['account.move.line'].search_read(domain=[('ref', '=', self.name),('display_type', '!=', 'payment_term')],fields=['name', 'quantity', 'price_unit', 'product_type'])
                    ...
                    if not account_move_line:
                        ...
                        for rec in account_move_line:
                            if rec['product_type'] == 'room':
                                if booking_dict['name'] == rec['name'] and booking_dict['price_unit'] == rec[
                                    'price_unit'] and booking_dict['quantity'] != rec['quantity']:
                                    booking_list.append(
    ...
    def get_details(self):
        ...
        staff = self.env['res.users'].search_count([
            ('groups_id', 'in', [
                self.env.ref('kx_hotel_management.hotel_group_admin').id,
                self.env.ref('kx_hotel_management.hotel_group_reception').id,
            ])
        ])
        ...
        booking_invoices = AccountMove.search([
            ('ref', 'like', 'BOOKING'),
            ('move_type', '=', 'out_invoice'),
            ('state', '=', 'posted')
        ])        
        total_revenue = sum(invoice.amount_total for invoice in booking_invoices.filtered(lambda x: x.payment_state == 'paid'))
        today_revenue = sum(invoice.amount_total for invoice in booking_invoices.filtered(lambda x: x.payment_state == 'paid' and x.date == fields.date.today()))
        pending_payment = sum(invoice.amount_residual for invoice in booking_invoices.filtered(lambda x: x.payment_state in ['not_paid', 'partial']))
