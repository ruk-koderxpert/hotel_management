...
class SaleOrderWizard(models.TransientModel):
    ...
    def action_sale_order_pdf(self):
        data = {'booking': self.generate_data()}
        return self.env.ref('kx_hotel_management.action_report_sale_order').report_action(self, data=data)
    ...
    def generate_data(self):
        ...
        room_booking = self.env['room.booking'].search_read(domain=domain, fields=['partner_id', 'name', 'checkin_date', 'checkout_date', 'amount_total'])
    ...
    def get_xlsx_report(self, data, response):
        ...
        cell_format = workbook.add_format({'font_size': '14px', 'bold': True, 'align': 'center', 'border': True})
        head = workbook.add_format({'align': 'center', 'bold': True, 'font_size': '23px', 'border': True})
