...
class MergeTicket(models.Model):
    ...
    user_id = fields.Many2one('res.partner', string='Responsible User',help='User name of responsible person.',
                              default=lambda self: self.env.user.partner_id.id)
    support_team_id = fields.Many2one('team.helpdesk', string='Support Team', help='Name of the support team.')
    customer_id = fields.Many2one('res.partner', string='Customer', help='Name of the Customer ')
    support_ticket_id = fields.Many2one('ticket.helpdesk', string='Support Ticket', help="Name of the support ticket")
    new_ticket = fields.Boolean(string='Create New Ticket?', help='Creating new tickets or not.')
    subject = fields.Char(string='Subject', help='Enter the New Ticket Subject')
    merge_reason = fields.Char(string='Merge Reason', help='Reason for Merging the tickets.')
    support_ticket_ids = fields.One2many('support.ticket', 'support_ticket_id', string='Support Tickets', help='Merged tickets')
    active = fields.Boolean(string='Disable Record', help='Disable Record', default=True)
    ...
    def _onchange_support_ticket_id(self):
        ...
        self.support_ticket_ids.write({'merged_ticket': self.support_ticket_id})
