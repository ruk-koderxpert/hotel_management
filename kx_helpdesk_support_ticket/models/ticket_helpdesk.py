...
class TicketHelpDesk(models.Model):
    ...
    def _default_show_create_task(self):
        return self.env['ir.config_parameter'].sudo().get_param('kx_helpdesk_support_ticket.show_create_task')

    def _default_show_category(self):
        return self.env['ir.config_parameter'].sudo().get_param('kx_helpdesk_support_ticket.show_category')

    name = fields.Char('Name', default=lambda self: self.env['ir.sequence'].next_by_code('ticket.helpdesk') or _('New'), help='Ticket Name')
    customer_id = fields.Many2one('res.partner', string='Customer Name', help='Customer Name')
    customer_name = fields.Char('Customer Name', help='Customer Name')
    subject = fields.Text('Subject', required=True, help='Subject of the Ticket')
    description = fields.Text('Description', required=True, help='Description')
    email = fields.Char('Email', help='Email')
    phone = fields.Char('Phone', help='Contact Number')
    team_id = fields.Many2one('team.helpdesk', string='Helpdesk Team', help='Helpdesk Team Name')
    product_ids = fields.Many2many('product.template', string='Product', help='Product Name')
    project_id = fields.Many2one('project.project', string='Project', readonly=False, related='team_id.project_id', store=True, help='Project Name')
    ...
    stage_id = fields.Many2one('ticket.stage', string='Stage', 
                               default=lambda self: self.env['ticket.stage'].search([('name', '=', 'Draft')], limit=1).id,
                               tracking=True,
                               group_expand='_read_group_stage_ids',
                               help='Stages', readonly=True)
    ...
    invoice_ids = fields.Many2many('account.move', string='Invoices', help='Invoicing id')
    task_ids = fields.Many2many('project.task', string='Tasks', help='Task id')
    ...
    ticket_type_id = fields.Many2one('helpdesk.type', string='Ticket Type', help='Ticket Type')
    team_head_id = fields.Many2one('res.users', string='Team Leader', compute='_compute_team_head_id', help='Team Leader Name')
    ...
    category_id = fields.Many2one('helpdesk.category', string='Category', help='Category')
    tags_ids = fields.Many2many('helpdesk.tag', help='Tags', string='Tags')
    assign_user = fields.Boolean(default=False, help='Assign User', string='Assign User')
    attachment_ids = fields.One2many('ir.attachment', 'res_id', help='Attachment Line', string='Attachments')
    merge_ticket_invisible = fields.Boolean(string='Merge Ticket', help='Merge Ticket Invisible or Not', default=False)
    ...
    create_task = fields.Boolean(string="Create Task", readonly=False, help='Create task or not', related='team_id.create_task', store=True)
    billable = fields.Boolean(string="Billable", default=False, help='Is billable or not', )
    ...
    service_level_agreement_id = fields.Many2one('service.level.agreement', string="SLA", help="Service Level Agreement")
    ticket_status_id = fields.Many2one('ticket.status', string="Ticket Status", help="Show Ticket Status")

    def action_done(self):
        done_stage = self.env['ticket.stage'].search([('name', '=', 'Done')], limit=1)
        self.stage_id = done_stage.id
        template = self.env.ref('kx_helpdesk_support_ticket.ticket_done_email_template')
        if template:
    ...
    def assign_to_teamleader(self):
        try:
            mail_template = self.env.ref('kx_helpdesk_support_ticket.odoo_website_helpdesk_assign')
            if not mail_template:
                raise ValidationError("Email template 'Ticket Assign' not found")
            ...
            mail_template.send_mail(
                ...
                email_values={
                    ...
                    'email_to': self.team_head_id.email,  # Use team_head_id email
                    'subject': f'Ticket Assignment: {self.name}'
                }
            )            
            return {
                ...
                'params': {
                    'title': _('Success'),
                    'message': _('Ticket assigned to team leader and notification sent'),
                    'sticky': False,
                    'type': 'success',
                }
            }
            ...
            raise ValidationError(f"Failed to send assignment email: {str(e)}")

    @api.onchange('team_id')
    def _onchange_team_id(self):
        if self.team_id:
            self.team_head_id = self.team_id.team_lead_id.id
            self.assign_to_teamleader()

    @api.onchange('assigned_user_id')
    def _onchange_stage(self):
        if self.assigned_user_id:
            assign = self.env['ticket.stage'].search([('name', '=', 'Assign')], limit=1)
            self.stage_id = assign
    ...
    def auto_close_ticket(self):
        auto_close = self.env['ir.config_parameter'].sudo().get_param('kx_helpdesk_support_ticket.auto_close_ticket')
        if auto_close:
            no_of_days = self.env['ir.config_parameter'].sudo().get_param('kx_helpdesk_support_ticket.no_of_days')
    ...
    def action_create_invoice(self):
        ...
        total = sum(task.effective_hours for task in tasks if task.effective_hours > 0)
        
        if not total:
            raise UserError('No billable hours found in the selected tasks')
        ...
        tasks.write({'ticket_billed': True})
    ...
    def action_open_merged_tickets(self):
        ticket_ids = self.env['support.ticket'].search([('merged_ticket', '=', self.id)])
        helpdesk_ticket_ids = ticket_ids.mapped('name')
        help_ticket_records = self.env['ticket.helpdesk'].search([('name', 'in', helpdesk_ticket_ids)])
        return {
            'type': 'ir.actions.act_window',
    ...
    def action_send_reply(self):
        template_id = self.env['ir.config_parameter'].sudo().get_param('kx_helpdesk_support_ticket.reply_template_id')
