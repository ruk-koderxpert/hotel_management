...
class HelpDeskDashboard(http.Controller):
    ...
    def helpdesk_dashboard(self):
        ...
        stage_ids = {
            name: request.env['ticket.stage'].search([('name', '=', name)],
                                                     limit=1).id for name in stage_names}
        new_stages = [stage_ids['Inbox'], stage_ids['Draft']]
        def get_ticket_data(stage_ids):
            tickets = request.env["ticket.helpdesk"].search([('stage_id', 'in', stage_ids)])
    ...
    def helpdesk_dashboard_week(self):
        ...
        stage_names = ['Inbox', 'Draft', 'In Progress', 'Canceled', 'Done', 'Closed']
        stages = {
            name: request.env['ticket.stage'].search([('name', '=', name)], limit=1).id for name in stage_names}
        stage_ids = [stages['Inbox'], stages['Draft']]
        def get_ticket_data(stage_id):
            count = request.env["ticket.helpdesk"].search_count([('stage_id', '=', stage_id), ('create_date', '>', week_ago)])
            ids = request.env["ticket.helpdesk"].search([('stage_id', '=', stage_id), ('create_date', '>', week_ago)]).ids
            return count, ids
        new_count, new_ids = get_ticket_data(stage_ids)
        in_progress_count, in_progress_ids = get_ticket_data(stages['In Progress'])
    ...
    def helpdesk_dashboard_month(self):
        ...
        domain_stage = ['Inbox', 'Draft', 'In Progress', 'Canceled', 'Done', 'Closed']
        stages = request.env['ticket.stage'].search([('name', 'in', domain_stage)])
        ...
        def get_stage_data(stage_names):
            ...
            tickets = request.env["ticket.helpdesk"].search([('stage_id', 'in', stage_ids_list), ('create_date', '>', week_ago)])
            return len(tickets), [ticket.id for ticket in tickets]
    ...
    def helpdesk_dashboard_year(self):
        ...
        stage_ids = {
            stage: request.env['ticket.stage'].search([('name', '=', stage)],
                                                      limit=1).id for stage in stages}
        def get_ticket_data(stage_name):
            stage_id = stage_ids[stage_name]
            tickets = request.env["ticket.helpdesk"].search([('stage_id', '=', stage_id), ('create_date', '>', year_ago)])
            return len(tickets), [ticket.id for ticket in tickets]
        dashboard_values = {}
        for stage in stages:
            count, ids = get_ticket_data(stage)
            dashboard_values.update({stage.lower(): count, f'{stage.lower()}_id': ids})
        return dashboard_values
