...
class TicketPortal(portal.CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'ticket_count' in counters:
            ticket_count = request.env['ticket.helpdesk'].search_count(
                self._get_tickets_domain()) if request.env['ticket.helpdesk'].check_access_rights('read', raise_exception=False) else 0
    ...
    def portal_my_tickets(self):
        ...
        return request.render("kx_helpdesk_support_ticket.portal_my_tickets", values)
    ...
    def portal_tickets_details(self, **kwargs):
        ...
        return request.render("kx_helpdesk_support_ticket.portal_ticket_details", data)

    @http.route('/my/tickets/download/<id>', auth='public', type='http', website=True)
    def ticket_download_portal(self, **kwargs):
        ...
        data = {'help': request.env['ticket.helpdesk'].sudo().browse(ticket_id)}
        report = request.env.ref('kx_helpdesk_support_ticket.report_ticket')
        pdf, _ = report.sudo()._render_qweb_pdf(report, res_ids=ticket_id, data=data)
        pdf_http_headers = [('Content-Type', 'application/pdf'),
                            ('Content-Length', len(pdf)),
                            ('Content-Disposition', 'attachment; filename="Helpdesk Ticket.pdf"')]
        return request.make_response(pdf, headers=pdf_http_headers)

# class TicketPortal(http.Controller):
...
#         return request.render('kx_helpdesk_support_ticket.portal_ticket_template', {
#             'ticket': ticket,
#         })